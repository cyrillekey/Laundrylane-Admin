...
import { getDeliveryZonesQueryKey } from "@/queries/@tanstack/react-query.gen";
import { deleteDeliveryZonesById } from "@/queries/sdk.gen";
import { TableActions } from "@/container/delivery-zones/table-actions";
...
const columnClass: Record<string, { head?: string; cell?: string }> = {
  ...
  location: { head: "px-4", cell: "px-4" },
  price: { head: "px-4", cell: "px-4" },
  radius: { head: "px-4", cell: "px-4" },
  hours: { head: "px-4", cell: "px-4 text-muted-foreground" },
...
export function DeliveryZonesTable({ zones, isPending }: DeliveryZonesTableProps) {
  ...
  const { mutateAsync: bulkDelete, isPending: bulkDeleting } = useMutation({
    mutationFn: async (ids: number[]) => {
      await Promise.all(ids.map((id) => deleteDeliveryZonesById({ path: { id } })));
    },
    onSuccess: () => {
    ...
    onError: (error) => {
      toast.error("Error!", { description: (error as Error)?.message || "Failed to delete delivery zones" });
    },
  });
  ...
  const columns: ColumnDef<DeliveryZoneItem>[] = [
    ...
    },
    {
      ...
      header: "Radius",
      cell: ({ row }) =>
        row.original.radius != null
          ? `${row.original.radius} km`
          : "—",
      enableSorting: false,
    },
    {
    ...
    <Card className="rounded-lg">
      ...
      {!isPending && table.getRowModel().rows.length > 0 && (
        <div className="flex items-center justify-between px-6 py-3 border-t">
          <p className="text-sm text-muted-foreground">
            {table.getPrePaginationRowModel().rows.length} zone
      ...
      <AlertDialog open={bulkDeleteOpen} onOpenChange={setBulkDeleteOpen}>
        <AlertDialogContent size="sm">
          <AlertDialogHeader>
            ...
            <AlertDialogTitle>Delete Delivery Zones</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to delete {selectedCount} delivery zone
